...
export default function Admin() {
  ...
  const processWithdrawal = useCallback(async (id, status) => {
    setProcessingWd(id);
    try {
      await fetch('/api/admin-referrals?action=process-withdrawal', {
        method: 'POST', headers: { 'Content-Type': 'application/json', ...H },
        body: JSON.stringify({ withdrawalId: id, status, adminUid: user?.uid }),
      });
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-8 space-y-4">
        ...
        {activeTab === 'referrals' && <div className="space-y-4 animate-in fade-in duration-200">
          ...
          {refLoading&&!refStats?<div className="flex justify-center py-10"><Loader2 className="animate-spin text-green-600" size={24} /></div>:refStats&&<>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">{[
              ...
              { l: 'Pending Rewards', v: `NGN ${(refStats.totalPending/100).toLocaleString()}`, c: 'text-amber-600' },
              { l: 'Paid Out', v: `NGN ${(refStats.totalRewardsPaid/100).toLocaleString()}`, c: 'text-green-600' },
              { l: 'Pending Payouts', v: refStats.pendingWithdrawals, c: 'text-orange-600' },
        ...
        {activeTab === 'withdrawals' && <div className="space-y-4 animate-in fade-in duration-200">
          ...
          {wdLoading?<div className="flex justify-center py-10"><Loader2 className="animate-spin text-green-600" size={24} /></div>:<div className="bg-white rounded-xl border border-gray-100 shadow-xs overflow-hidden"><div className="divide-y divide-gray-50">{withdrawals.length===0?<div className="p-6 text-center text-gray-400 text-sm">No withdrawal requests.</div>:withdrawals.map(w=><div key={w.id} className="px-4 py-3 hover:bg-gray-50/50"><div className="flex items-start justify-between gap-2"><div className="min-w-0 flex-1"><p className="text-sm font-bold text-gray-900 truncate">{w.storeName||'Unknown'}</p><p className="text-[10px] text-gray-500">{w.bankName} · {w.bankAccount}</p><p className="text-[10px] text-gray-400">{w.createdAt?new Date(w.createdAt).toLocaleString('en-NG'):'—'}</p></div><div className="text-right flex-shrink-0"><p className="text-base font-black text-gray-900">NGN {((w.amount||0)/100).toLocaleString()}</p><span className={`text-[9px] font-bold uppercase px-2 py-0.5 rounded-full inline-block mt-0.5 ${w.status==='completed'?'bg-green-100 text-green-700':w.status==='rejected'?'bg-red-100 text-red-700':'bg-amber-100 text-amber-700'}`}>{w.status}</span>{w.status==='pending'&&<div className="flex gap-1.5 mt-1.5 justify-end"><button onClick={()=>processWithdrawal(w.id,'completed')} disabled={processingWd===w.id} className="text-[10px] font-bold bg-green-600 text-white px-2.5 py-0.5 rounded-lg disabled:opacity-50">{processingWd===w.id?'...':'Approve'}</button><button onClick={()=>processWithdrawal(w.id,'rejected')} disabled={processingWd===w.id} className="text-[10px] font-bold bg-red-100 text-red-700 px-2.5 py-0.5 rounded-lg disabled:opacity-50">Reject</button></div>}</div></div></div>)}</div></div>}
        </div>}
