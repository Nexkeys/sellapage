    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-8 space-y-4">
        ...
        {activeTab === 'recovery' && <div className="space-y-4 animate-in fade-in duration-200">
          ...
          {recLoading ? <SkeletonRows count={5} /> : (
        ...
        {activeTab === 'directory' && <div className="space-y-3 animate-in fade-in duration-200">
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-xs overflow-hidden">
            ...
            <div className="block lg:hidden divide-y divide-gray-100">{(dirData?.stores||[]).map(s=><div key={s.id} className="p-3 space-y-2"><div className="flex justify-between items-start"><div className="min-w-0 flex-1"><p className="font-bold text-gray-900 text-sm truncate">{s.storeName||'Unnamed'}</p><p className="text-[10px] text-gray-400 font-mono">@{s.handle}</p></div><span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-full border flex-shrink-0 ${PLAN_C[s.plan]||PLAN_C.starter}`}>{s.plan}</span></div><div className="grid grid-cols-2 gap-1 text-[10px]"><span className="text-gray-500">W: {s.whatsappNumber||'-'}</span><span className="text-gray-500"><span className="font-bold text-gray-900">{s.listings?.total ?? 0}</span> listings ({s.listings?.products ?? 0}p · {s.listings?.services ?? 0}s)</span>{s.subaccountCode&&<span>{s.payoutsVerified?<span className="text-green-600 font-bold">Payout ✓</span>:<span className="text-amber-600 font-bold">Payout pending</span>}</span>}</div></div>)}</div>
        ...
        {activeTab === 'referrals' && <div className="space-y-4 animate-in fade-in duration-200">
          ...
          {refLoading&&!refStats?<SkeletonRows count={5} />:refStats&&<>
        ...
        {activeTab === 'tickets' && <div className="space-y-4 animate-in fade-in duration-200">
          ...
          {ticketLoading?<SkeletonRows count={5} />:<div className="bg-white rounded-xl border border-gray-100 shadow-xs overflow-hidden"><div className="divide-y divide-gray-50">{tickets.length===0?<div className="p-6 text-center text-gray-400 text-sm">No tickets found.</div>:tickets.map(t=><div key={t.id} className="px-4 py-3 hover:bg-gray-50/50">
        ...
        {activeTab === 'sella-ai' && <div className="space-y-4 animate-in fade-in duration-200">
          ...
          {sellaLoading?<SkeletonRows count={5} />:sellaData&&<>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">{[
        ...
        {activeTab === 'reports' && <div className="space-y-4 animate-in fade-in duration-200">
          ...
          {reportsLoading?<SkeletonRows count={5} />:<div className="bg-white rounded-xl border border-gray-100 shadow-xs overflow-hidden">
        ...
        {activeTab === 'jobs' && <div className="space-y-4 animate-in fade-in duration-200">
          ...
          {jobsLoading?<SkeletonRows count={5} />:<div className="bg-white rounded-xl border border-gray-100 shadow-xs overflow-hidden">
