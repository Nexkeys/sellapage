...
const TIKTOK_ADS_DOC = 'tiktokAds'
...
// ------------------------------------------------------------------- TikTok
// Events API access token. Generated by the VENDOR in their own TikTok Ads
// Manager (Tools > Events > their pixel > Settings > Generate Access Token) and
// pasted into the TikTok Pixel tab.
//
// Unlike `tiktokPixelId`, which is public by design and sits on the store
// document exactly like `metaPixelId`, this token can write conversion events
// into that vendor's advertising account. It is a credential, so it goes
// straight into private/* and has never touched the public doc - which is why,
// unlike the Google Ads and referral-bank helpers above, there is deliberately
// no lazy-migration fallback here. There is nothing to migrate FROM.

export async function getTikTokEventsToken(db, storeId) {
  const snap = await privateDoc(db, storeId, TIKTOK_ADS_DOC).get()
  return (snap.exists && snap.data().eventsToken) || null
}

export async function setTikTokEventsToken(db, storeId, eventsToken) {
  await privateDoc(db, storeId, TIKTOK_ADS_DOC).set(
    { eventsToken, updatedAt: new Date() },
    { merge: true },
  )
}

export async function clearTikTokEventsToken(db, storeId) {
  await privateDoc(db, storeId, TIKTOK_ADS_DOC).set(
    { eventsToken: null, updatedAt: new Date() },
    { merge: true },
  )
}

// Login Kit refresh token, from the vendor connecting their TikTok account.
//
// Shares the tiktokAds private doc with the Events token above: both are TikTok
// credentials for the same store, and one document read serves both.
//
// TikTok ROTATES the refresh token on every use, so setTikTokAuth is called on
// each refresh, not only at connect time. Failing to persist the rotated value
// leaves the connection working for about a day and then dying with no obvious
// cause, which is exactly the kind of silent failure this codebase has been
// bitten by before.

export async function getTikTokAuth(db, storeId) {
  const snap = await privateDoc(db, storeId, TIKTOK_ADS_DOC).get()
  if (!snap.exists) return null
  const d = snap.data()
  return d.refreshToken ? { refreshToken: d.refreshToken, openId: d.openId || null } : null
}

export async function setTikTokAuth(db, storeId, { refreshToken, openId }) {
  await privateDoc(db, storeId, TIKTOK_ADS_DOC).set(
    { refreshToken, openId: openId || null, updatedAt: new Date() },
    { merge: true },
  )
}

export async function clearTikTokAuth(db, storeId) {
  await privateDoc(db, storeId, TIKTOK_ADS_DOC).set(
    { refreshToken: null, openId: null, updatedAt: new Date() },
    { merge: true },
  )
}
