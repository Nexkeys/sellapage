...
#### Fixed
- **`src/utils/adminRoles.js`** — Added tab access for `cac`, `domains`, `announcements`, `tickets`, `analytics`, `revenue` to TAB_ACCESS. Marketing role now gets `directory`, `tickets`, `analytics`. Support role gets `tickets`.
- **`src/api-handlers/admin-health.js`** — NVIDIA AI Engine now returns `today`, `thisWeek`, `thisMonth` counts (not just total). Vercel section now fetches recent deployments from Vercel API (if `VERCEL_TOKEN` is set) showing commit messages, branch, and state. Fixed stray `};` on line 349 that caused `SyntaxError: Missing catch or finally after try` → all health/directory API calls returned 500.
- **`src/api-handlers/admin-referrals.js`** — Removed dead `authHeader`/`idToken` code. Fixed `processedAt` to use `new Date()` instead of ISO string. Added `approvedBy`, `approvedAt`, `paidAt` audit log fields on process-withdrawal. Added payout confirmation email via Resend on completed withdrawal. Resolves store name from `referralRewards` → `referredUserId` → `stores/{uid}` for display. Added highest earner calculation (computed from `referralRewards`, grouped by `referrerId`, sorted by total earned). Fixed `sendEmail` call to use positional args `(to, subject, html)` not object syntax.
- **`src/api-handlers/admin-manage.js`** — Fixed `.update()` crash on non-existent UID (now checks doc exists first). Fixed `.set()` overwrite (now checks if admin already exists, returns 409). Added role validation against `VALID_ROLES` array. Added `.trim()` on uid. Added boolean coercion for `active` field. Added `create-user` action: creates Firebase Auth user + saves to Firestore `admins` collection using `getAdminAuth()`.
- **`src/pages/Admin.jsx`** — Fixed missing argument in `navigator.clipboard.writeText()` (was `writeText)`, now `writeText(t)`).
...
- **`src/api-handlers/admin-announcements.js`** — Announcement/Banner System (replaced feature flags — feature flags were write-only, nothing read them): `list` (all announcements), `active` (active announcements for vendors), `create` (new announcement), `update` (edit), `delete`. Uses Firestore `announcements` collection.
- **`src/api-handlers/admin-tickets.js`** — Support Tickets: `list` (paginated from `supportMessages`, filterable by status, shows whatsappNumber/email/plan/category/storeId, priority-sorted: premium first), `update` (change status, assign).
- **`src/api-handlers/admin-analytics.js`** — Platform Analytics: `overview` (real counts from Firestore), `top-stores` (uses `collectionGroup('analytics')` for totalViews/totalClicks/engagedViews — real data, not fake), `signups` (daily series).
- **`src/api-handlers/admin-revenue.js`** — Revenue: `platform` (Paystack `/balance` + `/transaction/totals`), `transactions` (Paystack with date filters), `store-revenue` (Firestore orders ranked by total).
- **`api/[...route].js`** — Registered `admin-cac`, `admin-domains`, `admin-announcements`, `admin-tickets`, `admin-analytics`, `admin-revenue`.

#### Changed — Admin.jsx Full Rewrite
- **`src/pages/Admin.jsx`** — Complete rewrite with 11 tabs: Health, Merchants, Referrals, Payouts, CAC Verification, Custom Domains, Announcements, Support Tickets, Analytics, Revenue, Team. Mobile responsive with hamburger menu, bottom nav bar (11 tabs), tab-specific colors, pagination. All currency shown as "NGN" (not ₦).
...
### [2026-07-18] Admin Panel — Clipboard Fix + Build Pass [DONE]

#### Fixed
- **`src/pages/Admin.jsx`** — Fixed `navigator.clipboard.writeText)` → `navigator.clipboard.writeText(t)` syntax error (missing argument caused `SyntaxError: Expected ";" but found ")")`.
